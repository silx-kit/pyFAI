...
//    bin_bounds = (int2) *(col_ptr+bin_num);  // cool stuff!
    ...
    int workgroup_size = get_local_size(0); // or WORKGROUP_SIZE
    int num_of_workgroups = bin_size/workgroup_size  // + (bin_size%workgroup_size)*1;
	float sum_data = 0.0f;
...
// REMEMBER TO PASS WORKGROUP_SIZE AS A CPP DEF
        ...
        if (thread_id_loc == 0)
        {
    		...
    		if (sum_count > epsilon)
    			outMerge[bin_num] =  sum_data / sum_count;
    		else
