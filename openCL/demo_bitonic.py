...
evt.wait()
print("Execution time: %s ms, err=%s " % (1e-6 * (evt.profile.end - evt.profile.start), abs(numpy.sort(h_data) - d_data.get()).max()))
d_data = pyopencl.array.to_device(queue, h_data)
...
evt.wait()
print("Execution time: %s ms, err=%s " % (1e-6 * (evt.profile.end - evt.profile.start), abs(h2s_data - d2_data.get()).max()))
