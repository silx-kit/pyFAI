...
class test_mask(unittest.TestCase):
        ...
        for platform in range(10):
          for device in range(10):
            if not inte.init("all",platformid=platform, deviceid=device, useFp64=True):
              ...
              if "cl_khr_int64_base_atomics" in inte.get_device_info()["extensions"] or "NVIDIA CUDA" in inte.get_platform_info()["name"]:
                return ids        
    ...
    def test_OpenCL(self):
        ...
        for ds in self.datasets:
            ...
            ocl = ai.xrpd_OpenCL(data, 1000,platformid=ids[0],deviceid=ids[1],useFp64=True)
            t0 = time.time()
            ref = ai.xrpd(data, 1000)
            t1 = time.time()
            ocl = ai.xrpd_OpenCL(data, 1000,platformid=ids[0],deviceid=ids[1],useFp64=True)
            t2 = time.time()
            logger.info("For image image %s; speed up is %.3fx; rate is %.3f Hz" % (os.path.basename(ds["img"]), ((t1 - t0) / (t2 - t1)), 1./(t2 - t1)) )
            r = Rwp(ref, ocl)
