...
cdef inline double integrate(double A0, double B0, Line AB) nogil:
    """
...
class FullSplitCSR_1d(object):
    ...
    * indptr: row pointer indicates the start of a given row. len nrow+1
    ...
    def calc_lut(self):
        ...
        """
        cdef:
            double[:, :, :] cpos = numpy.ascontiguousarray(self.pos, dtype=numpy.float64)
            numpy.int8_t[:] cmask
            ...
            double pos0_min, pos0_max, pos0_maxin, pos1_min, pos1_max, pos1_maxin
            double max0, min0
            double areaPixel, delta, areaPixel2
            double A0, B0, C0, D0, A1, B1, C1, D1
            double A_lim, B_lim, C_lim, D_lim
            double oneOverArea, partialArea, tmp
            Line AB, BC, CD, DA
            int bins, i, idx, bin, bin0, bin0_max, bin0_min, bin1_min, pixel_bins, k, size
        ...
        with nogil:
            for idx in range(size):
                ...
                A0 = getBinNr(cpos[idx, 0, 0], pos0_min, delta)
                A1 = cpos[idx, 0, 1]
                B0 = getBinNr(cpos[idx, 1, 0], pos0_min, delta)
                B1 = cpos[idx, 1, 1]
                C0 = getBinNr(cpos[idx, 2, 0], pos0_min, delta)
                C1 = cpos[idx, 2, 1]
                D0 = getBinNr(cpos[idx, 3, 0], pos0_min, delta)
                D1 = cpos[idx, 3, 1]
        ...
        cdef numpy.ndarray[numpy.int32_t, ndim = 1] indices = numpy.zeros(nnz, dtype=numpy.int32)
        cdef numpy.ndarray[numpy.float32_t, ndim = 1] data = numpy.zeros(nnz, dtype=numpy.float32)
        ...
        with nogil:
            for idx in range(size):
                ...
                if bin0_min == bin0_max:
                    ...
                    # offseting the min bin of the pixel to be zero to avoid precision problems
                    A0 -= bin0_min
                    B0 -= bin0_min
                    C0 -= bin0_min
                    D0 -= bin0_min
                    ...
                    for bin in range(bin0_min, bin0_max + 1):
                        bin0 = bin - bin0_min
                        # Complicated vectorized formula by Giannis
    ...
    def integrate(self, weights, dummy=None, delta_dummy=None, dark=None, flat=None, solidAngle=None, polarization=None):
        ...
        """
        cdef:
            numpy.int32_t i, j, idx, bins = self.bins, size = self.size
            float sum_data, sum_count, epsilon=1e-10
            float data, coef, cdummy, cddummy
