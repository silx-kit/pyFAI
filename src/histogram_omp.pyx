...
def histogram(numpy.ndarray pos not None, \
    ...
    cdef:
        ...
        numpy.ndarray[numpy.float64_t, ndim = 1] out_pos = numpy.zeros(bins, dtype="float64")
        double delta, bin_edge_min, bin_edge_max
        double a = 0.0
        double d = 0.0
        double fbin = 0.0
        double ffbin = 0.0
...
#            big_data[thread, bin] += d 
        ...
        for bin in prange(bins):
            ...
            out_pos[bin] += bin_edge_min + (0.5 +< double > bin) * delta
            tmp_count = 0.0
            tmp_data = 0.0
...
#    if weights is not No:
    ...
    cdef:
        ...
        numpy.ndarray[numpy.float64_t, ndim = 2] out_merge = numpy.zeros((bin0, bin1), dtype="float64")
